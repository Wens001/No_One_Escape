...
public class uTransformEditor : Editor {
    ...
    public override void OnInspectorGUI()
    {
        ...
        //Draw the Utilities
        SHOW_UTILS = uEditorUtils.DrawHeader("Transform 实用工具");
        if (SHOW_UTILS)
            DrawUtilities();
    ...
    {
        GUILayout.Space(18);
        if (GUILayout.Button(new GUIContent("", icon_revert, "重置此对象位置"), uEditorUtils.uEditorSkin.GetStyle("ResetButton"), GUILayout.Width(18), GUILayout.Height(18)))
        {
    ...
    {
        GUILayout.Space(18);
        if (GUILayout.Button(new GUIContent("", icon_revert, "重置此对象旋转"), uEditorUtils.uEditorSkin.GetStyle("ResetButton"), GUILayout.Width(18), GUILayout.Height(18)))
        {
    ...
    private void DrawScaleReset()
    {
        ...
        if (GUILayout.Button(new GUIContent("", icon_revert, "重置此对象大小"), uEditorUtils.uEditorSkin.GetStyle("ResetButton"), GUILayout.Width(18), GUILayout.Height(18)))
        {
    ...
    private void DrawUtilities()
    {
        ...
        //Snap to ground
        GUILayout.BeginHorizontal();
        if (GUILayout.Button("贴合到地面", uEditorUtils.uEditorSkin.button, GUILayout.Width(ThinInspectorMode ? 100 : 160)))
        {
        ...
        //Random rotation
        GUILayout.BeginHorizontal();
        if (GUILayout.Button("随机旋转", uEditorUtils.uEditorSkin.button, GUILayout.Width(ThinInspectorMode ? 100 : 160), GUILayout.Height(EditorGUIUtility.singleLineHeight * 2)))
        {
            foreach (var tar in this.targets)
            {
                Transform t = (Transform)tar;
        ...
        //位置复制黏贴
        GUILayout.BeginHorizontal();
        if (GUILayout.Button("复制位置", uEditorUtils.uEditorSkin.button, GUILayout.Width(160), GUILayout.Height(EditorGUIUtility.singleLineHeight * 2)))
        {
            foreach (var tar in this.targets)
            {
                Transform t = (Transform)tar;
                copyPosition = t.position;
                copyRotation = t.rotation;
                hasCopy = true;
                break;
            }
        }

        if (hasCopy && GUILayout.Button("黏贴位置", uEditorUtils.uEditorSkin.button, GUILayout.Width(160), GUILayout.Height(EditorGUIUtility.singleLineHeight * 2)))
        {
            foreach (var tar in this.targets)
            {
                Transform t = (Transform)tar;
                Undo.RecordObject(t, "Parse Transfrom");
                t.position = copyPosition ;
                t.rotation = copyRotation ;
            }
        }
        GUILayout.EndHorizontal();
        EditorGUIUtility.labelWidth = 0;
