    ...
    public class LocalizedAssetEditor : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            if (m_LocaleItems != null)
            {
                ...
                m_LocaleItemList.onAddDropdownCallback = (Rect buttonRect, ReorderableList list) =>
                {
                    ...
                    menu.AddItem(new GUIContent("添加一种语言", "添加一种语言."), false, () =>
                    {
                        ReorderableList.defaultBehaviours.DoAddButton(list);
                        serializedObject.ApplyModifiedProperties();
                    });
                    menu.AddItem(new GUIContent("从设置添加语言", "Adds by searching used languages in assets."), false, () =>
                    {
                        AddLanguagesFromSettings();
                        serializedObject.ApplyModifiedProperties();
                    });
                    menu.AddItem(new GUIContent("添加所有语言", "Adds all languages."), false, () =>
                    {
                        AddAllLanguages();
                        serializedObject.ApplyModifiedProperties();
                    });
        ...
        public override void OnInspectorGUI()
        {
            if (m_LocaleItemList != null)
            {
                ...
                EditorGUI.HelpBox(helpRect, "如果需要，第一个区域设置项用作回退.", MessageType.Info);
            }
            else
            {
                base.OnInspectorGUI();
                
                EditorGUILayout.HelpBox("确保locale items变量名声明为\"m_LocaleItems\"，并且是可序列化的.",
                    MessageType.Error);
            }
        }
