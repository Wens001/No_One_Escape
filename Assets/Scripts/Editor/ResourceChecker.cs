 ...
 * Author:			
 * Email:			
 * CreateTime:		2020-05-02-15:00:59
 * Version:			1.0
...
public class ResourceChecker : EditorWindow {
	...
	{
		defColor = GUI.color;
		IncludeDisabledObjects = GUILayout.Toggle(IncludeDisabledObjects, "包括禁用的对象", GUILayout.Width(300));
		IncludeSpriteAnimations = GUILayout.Toggle(IncludeSpriteAnimations, "在精灵动画中查找", GUILayout.Width(300));
		GUI.color = new Color (0.8f, 0.8f, 1.0f, 1.0f);
		IncludeScriptReferences = GUILayout.Toggle(IncludeScriptReferences, "查看行为字段", GUILayout.Width(300));
		GUI.color = new Color (1.0f, 0.95f, 0.8f, 1.0f);
		IncludeGuiElements = GUILayout.Toggle(IncludeGuiElements, "在GUI元素中查找", GUILayout.Width(300));
		IncludeLightmapTextures = GUILayout.Toggle(IncludeLightmapTextures, "在光照贴图纹理中查找", GUILayout.Width(300));
		GUI.color = defColor;
		GUILayout.BeginArea(new Rect(position.width-85,5,100,65));
		if (GUILayout.Button("刷新", GUILayout.Width(80), GUILayout.Height(40)))
			CheckResources();
		...
		if (thingsMissing == true) {
			EditorGUI.HelpBox (new Rect(8,93,300,25), "一些游戏对象缺少图形元素.", MessageType.Error);
		}
