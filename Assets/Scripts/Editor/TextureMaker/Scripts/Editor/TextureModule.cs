...
namespace TextureMaker
{
    ...
    public class TextureModule
    {
        ...
        public void DrawTextureControls()
        {
            ...
            textureSettingsFoldout = EditorGUILayout.BeginFoldoutHeaderGroup(textureSettingsFoldout, "设置", foldoutStyle);
            
            if(textureSettingsFoldout)
            {
                EditorGUI.indentLevel++;

                moduleType = (TextureModuleType)EditorGUILayout.EnumPopup("类型", moduleType);

                GetModule().Draw();

                invert = EditorGUILayout.Toggle("倒置", invert);

                textureSize = EditorGUILayout.Vector2IntField("大小", textureSize);
    ...
    public class GradientModule : ITextureModule
    {
        ...
        public void Draw()
        {
            gradientType = (GradientType)EditorGUILayout.EnumPopup("填充", gradientType);

            gradient = EditorGUILayout.GradientField("颜色", gradient);

            if(gradientType == GradientType.RADIAL)
            {
                radialMaskThreshold = EditorGUILayout.FloatField("遮罩阈值", radialMaskThreshold);
            ...
            flip = EditorGUILayout.Toggle("翻转", flip);
    ...
    public class NoiseModule : ITextureModule
    {
        ...
        public void Draw()
        {
            noiseType = (NoiseType)EditorGUILayout.EnumPopup("填充", noiseType);
            ...
            if(noiseType == NoiseType.PERLIN)
            {
                ...
                perlinNoiseScale = EditorGUILayout.Slider("大小", perlinNoiseScale, 0.01f, 1000f);
                perlinNoisePersistence = EditorGUILayout.Slider("Persistence", perlinNoisePersistence, 0f, 1f);
                seed = EditorGUILayout.IntSlider("种子", seed, 1, 100000);
                perlinNoiseOffset = EditorGUILayout.Vector2Field("偏移", perlinNoiseOffset);
                
                flip = EditorGUILayout.Toggle("填充", flip);
            }
            else if(noiseType == NoiseType.RANDOM)
            {
                seed = EditorGUILayout.IntSlider("种子", seed, 1, 100000);
                useColors = EditorGUILayout.Toggle("使用颜色", useColors);
            }
            else if(noiseType == NoiseType.VORONOI)
            {
                voronoiDstType = (VoronoiDistanceMetric)EditorGUILayout.EnumPopup("距离类型", voronoiDstType);
                sites = EditorGUILayout.IntField("地点", sites);
                seed = EditorGUILayout.IntSlider("种子", seed, 1, 100000);
                useColors = EditorGUILayout.Toggle("使用颜色", useColors);
                renderFlat = EditorGUILayout.Toggle("渲染平面", renderFlat);
    ...
    public class PatternModule : ITextureModule
    {
        ...
        public void Draw()
        {
            patternType = (PatternType)EditorGUILayout.EnumPopup("类型", patternType);

            color1 = EditorGUILayout.ColorField("颜色1", color1);
            color2 = EditorGUILayout.ColorField("颜色2", color2);

            count = EditorGUILayout.Vector2IntField("数量", count);
            ...
            {
                padding = EditorGUILayout.Vector2Field("填充", padding);
            }
            else if(patternType == PatternType.CIRCLES)
            {
                scale = EditorGUILayout.Slider("大小", scale, 1, 100);
            }
