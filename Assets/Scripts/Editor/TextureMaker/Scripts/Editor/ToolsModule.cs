...
namespace TextureMaker
{
    ...
    public class ToolsModule
    {
        ...
        public void DrawPreviewControls(PreviewType previewType)
        {
            if(previewType == PreviewType.NORMAL_MAP || previewType == PreviewType.SOBEL_OPERATOR)
            {
                ...
                strength = EditorGUILayout.Slider("力量", strength, 0.0f, 5.0f);
            }
        }
                ...
                for(int i = 0; i < texturesList.Count; i++)
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        if(i == 0)
                        {
                            ...
                            {
                                EditorGUILayout.HelpBox("纹理必须大小相等，才能混合工作.", MessageType.Info);
                                useAlphaTextures = EditorGUILayout.Toggle("使用alpha纹理", useAlphaTextures);
                            }
                        ...
                        if(i > 0)
                        {
                            ...
                            if(useAlphaTextures)
                            {
                                EditorGUILayout.LabelField(string.Format("遮罩 {0}", (i)), EditorStyles.centeredGreyMiniLabel, GUILayout.Width(50));
                            ...
                            else
                            {
                                ...
                                if(texturesList[i])
                                {
                                    ...
                                    EditorGUILayout.BeginHorizontal();
                                    {
                                        EditorGUILayout.LabelField("混合 " + i, EditorStyles.centeredGreyMiniLabel, GUILayout.MaxWidth(40));
                        ...
                        GUILayout.FlexibleSpace();                                     
                        EditorGUILayout.LabelField(string.Format("纹理 {0}", i + 1), EditorStyles.centeredGreyMiniLabel, GUILayout.Width(50));
    ...
    {        
        ...
        public void Draw()
        {
            ...
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField(string.Format("纹理"), EditorStyles.centeredGreyMiniLabel, GUILayout.Width(40));
