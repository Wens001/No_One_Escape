...
namespace BezierCurves
{
    ...
    public class BezierCurve3DEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("输出长度"))
            {
                Debug.Log(this.curve.GetApproximateLength());
            }
            if (GUILayout.Button("以开始点为原点闭合曲线"))
            {
                curve.ConnectStartEndPoint(true);
            }
            if (GUILayout.Button("以末尾点为原点闭合曲线"))
            {
                curve.ConnectStartEndPoint(false);
            }

            this.showPoints = EditorGUILayout.Foldout(this.showPoints, "Key Points");
            if (this.showPoints)
            {
                if (GUILayout.Button("增加点"))
                {
                    AddKeyPointAt(this.curve, this.curve.KeyPointsCount);
                }

                if (GUILayout.Button("增加点并选择"))
                {
        ...
        private void DrawElementCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (GUI.Button(new Rect(rect.x, rect.y, AddButtonWidth, EditorGUIUtility.singleLineHeight), new GUIContent("前加")))
            {
            ...
            if (GUI.Button(new Rect(rect.width - AddButtonWidth + 8f, rect.y, AddButtonWidth, EditorGUIUtility.singleLineHeight), new GUIContent("后加")))
            {
