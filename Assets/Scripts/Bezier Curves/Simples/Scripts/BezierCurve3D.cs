...
namespace BezierCurves
{
    public class BezierCurve3D : MonoBehaviour
    {
        ...
        /// 连接首尾部
        /// </summary>
        public void ConnectStartEndPoint(bool isStart)
        {
            if (keyPoints.Count <= 1)
                return;
            var start = isStart ?  keyPoints[0] : keyPoints[keyPoints.Count - 1];
            var end = isStart ? keyPoints[keyPoints.Count-1] : keyPoints[0] ;
            end.transform.position = start.transform.position;
            end.transform.rotation = start.transform.rotation;
            end.LeftHandleLocalPosition = start.LeftHandleLocalPosition;
            end.RightHandleLocalPosition = start.RightHandleLocalPosition;
        }
